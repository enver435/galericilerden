/*
    ...
    # @Last Modified time: 2017-09-25 08:56:38
*/
...
function getPreview()
{
    ...
    var adPriceType    = $('input[name=price_type]').val();
...
function dopingPayment()
{
    var serialize = $('form[name=doping-card]').serialize();
    var dopings   = $('form[name=ad-add] input[name=dopings], form[name=ad-update] input[name=dopings]').val();
    ...
    if(ajaxLock === false)
    {
        ...
        $.ajax({
            ...
            url: siteUrl + '/request',
            dataType: 'json',
            data: serialize + '&mode=dopingPayment&totalPrice=' + totalPrice + '&dopings=' + dopings,
            success: function(response)
            {
                if(response.error)
                {
                    ...
                    $('form[name=doping-card]').find('input').each(function(idx, elem) {
                ...
                {
                    $('form[name=ad-add]').submit();
                    $('form[name=ad-update]').submit();
                }
...
function limitPayment()
{
    var serialize = $('form[name=limit-card]').serialize();
    ...
    if(ajaxLock === false)
    {
        ...
        $.ajax({
            ...
            url: siteUrl + '/request',
            dataType: 'json',
            data: serialize + '&mode=limitPayment&limitPrice=' + limitPrice,
            success: function(response)
            {
                if(response.error)
                {
                    ...
                    $('form[name=limit-card]').find('input').each(function(idx, elem) {
...
function getStorePayment()
{
    var firmName = $('input[name=firm_name]').val();

    if(firmName == '')
    {
        $('input[name=firm_name]').parent().addClass('has-error');
        $('input[name=sales_name]').parent().addClass('has-error');
        $('input[name=sales_surname]').parent().addClass('has-error');
        $('input[name=sales_phone]').parent().addClass('has-error');
    }
    else
    {
        $('input[name=firm_name]').parent().removeClass('has-error');
        $('input[name=sales_name]').parent().removeClass('has-error');
        $('input[name=sales_surname]').parent().removeClass('has-error');
        $('input[name=sales_phone]').parent().removeClass('has-error');

        $('.add-store-firms').hide();
        $('.add-store-payment').show();

        $('.step-three').addClass('active');
        $('.step-three').find('span').addClass('active');
    }
}

function storeCreatePayment()
{
    var serialize = $('form[name=storeCreate-card]').serialize();
    var storeId   = $('select[name=store_type] option:selected').val();
    ...
    if(ajaxLock === false)
    {
        ...
        $.ajax({
            ...
            url: siteUrl + '/request',
            dataType: 'json',
            data: serialize + '&mode=storeCreatePayment&storeId=' + storeId + '&storePaymentPrice=' + storePaymentPrice,
            success: function(response)
            {
                if(response.error)
                {
                    ...
                    $('form[name=storeCreate-card]').find('input').each(function(idx, elem) {
...
function storeUpdateLimitPayment()
{
    var serialize = $('form[name=storelimitupdate-card]').serialize();
    var storeId   = $('select[name=store_type] option:selected').val();
    ...
    if(ajaxLock === false)
    {
        ...
        $.ajax({
            ...
            url: siteUrl + '/request',
            dataType: 'json',
            data: serialize + '&mode=storeUpdateLimitPayment&storeId=' + storeId + '&storePaymentPrice=' + storePaymentPrice,
            success: function(response)
            {
                if(response.error)
                {
                    ...
                    $('form[name=storelimitupdate-card]').find('input').each(function(idx, elem) {
                ...
                {
                    $('form[name=store-limit-update]').submit();
                }
...
function storeUpdateYearPayment()
{
    var serialize = $('form[name=storeyearupdate-card]').serialize();
    var storeId   = $('select[name=store_type] option:selected').val();
    ...
    if(ajaxLock === false)
    {
        ...
        $.ajax({
            ...
            url: siteUrl + '/request',
            dataType: 'json',
            data: serialize + '&mode=storeUpdateYearPayment&storeId=' + storeId + '&storePaymentPrice=' + storePaymentPrice,
            success: function(response)
            {
                if(response.error)
                {
                    ...
                    $('form[name=storeyearupdate-card]').find('input').each(function(idx, elem) {
                ...
                {
                    $('form[name=store-year-update]').submit();
                }

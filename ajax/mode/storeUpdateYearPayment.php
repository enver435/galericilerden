<?php
	...
	# @Last Modified time: 2017-08-31 16:06:29
		...
		if($storeInfo !== false)
		{
			if($cardNumber != '' AND strlen($cardNumber) >= 16 AND $cardHolderName != '' AND strlen($cardExpiry) >= 4 AND strlen($cardCvc) >= 3 AND $storeId != '' AND $storePaymentPrice > 0 AND $storePaymentPrice == $storeInfo['store_price'])
			{
				...
				if(!empty($userInfo))
				{
					...
					$basketItem->setId('Mağaza Yıllık Güncelleme (' . $storeInfo['store_type_name'] . ')');
					$basketItem->setName('Mağaza Yıllık Güncelleme (' . $storeInfo['store_type_name'] . ')');
					$basketItem->setCategory1('Mağaza Yıllık Güncelleme (' . $storeInfo['store_type_name'] . ')');
					$basketItem->setCategory2('Mağaza Yıllık Güncelleme (' . $storeInfo['store_type_name'] . ')');
					...
					{
						setSession('storeYearUpdatePaymentSuccess', true); // odenisi success edek ki post edende orda yoxlayacam
